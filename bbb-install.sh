...
OPTIONS (install BigBlueButton):
  ...
  -p <host>              Use aptitude proxy at <host>
...
main() {
  ...
  if [ "$DISTRO" == "xenial" ]; then 
    ...
    if dpkg -s nodejs | grep Version | grep -q 4.2.6; then
      aptitude purge -y nodejs > /dev/null 2>&1
    fi
    aptitude purge -yq kms-core-6.0 kms-elements-6.0 kurento-media-server-6.0 > /dev/null 2>&1  # Remove older packages
...
need_pkg() {
  ...
  if ! dpkg -s ${@:1} >/dev/null 2>&1; then
    LC_CTYPE=C.UTF-8 aptitude install -yq ${@:1}
  fi
}
...
install_greenlight(){
  ...
  if dpkg -l | grep -q docker-compose; then
    aptitude purge -y docker-compose
  fi
